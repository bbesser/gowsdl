...
{{range .}}
	...
	{{range .Operations}}
		...
		{{$soapAction := findSOAPAction .Name $privateType}}
		{{$responseType := findType .Output.Message | replaceReservedWords | makePublic}}
		func (service *{{$privateType}}) {{makePublic .Name | replaceReservedWords}}Context (ctx context.Context, {{if ne $requestType ""}}request *{{$requestType}}WithNamespace{{end}}) ({{if ne $responseType ""}}*{{$responseType}}, {{end}}error) {
		...
		func (service *{{$privateType}}) {{makePublic .Name | replaceReservedWords}} ({{if ne $requestType ""}}request *{{$requestType}}WithNamespace{{end}}) ({{if ne $responseType ""}}*{{$responseType}}, {{end}}error) {
